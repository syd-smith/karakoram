...
# %%
noise = get_data('TSK', 5, 'd03', 'noise')
ctl = get_data('TSK', 5, 'd03', 'ctl')
...
open_elevation = Dataset(current_file_directory / 'wrfout' / f'wrfout_ctl' / '2016' / f'wrfout_d03_2016-05-01_00:00:00')
...
# see land use categories here: https://www.scielo.org.mx/scielo.php?script=sci_arttext&pid=S1405-55462023000200435
# create custom color map for land use categories
colors = plt.cm.jet(np.linspace(0, 1, 25))
# call out number 21 as black for water bodies (MODIS WRF land use classifications)
colors[21] = [0, 0, 0, 1] 
cmap = mcolors.ListedColormap(colors)
norm = mcolors.BoundaryNorm(np.arange(0, 26, 1), cmap.N)

# plot land use data
mesh = ax.pcolormesh(land_use['XLONG_M'], land_use['XLAT_M'], land_use.values, cmap = cmap, norm = norm, transform = ccrs.PlateCarree())
plt.colorbar(mesh, ax = ax,label = 'Land Use Classification', ticks = np.arange(0, 25, 1), orientation = 'horizontal', pad = 0.05)
plt.show()
